defmodule TrackerWeb.ManageController do
  ...
  def index(conn, _params) do
    manages = Taskfeed.list_manages()
    render(conn, "index.json", manages: manages)
  end

  def create(conn, %{"manage" => manage_params}) do
    with {:ok, %Manage{} = manage} <- Taskfeed.create_manage(manage_params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id, "manage" => manage_params}) do
    ...
    with {:ok, %Manage{} = manage} <- Taskfeed.update_manage(manage, manage_params) do
      render(conn, "show.json", manage: manage)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
    manage = Taskfeed.get_manage!(id)
    with {:ok, %Manage{}} <- Taskfeed.delete_manage(manage) do
